...
cdef class MinHeap:
	...
	cdef void insertKey(self, int k, int[:] degres):
		...
		cdef int p = parent(i)
		while (p >= 0) and (degres[self.arr[p]] > degres[self.arr[i]]) :
			self.swap(i, p)
			i = p
			p = parent(i)
	...
	cdef void minHeapify(self, int i, int[:] degres):
		...
		r = right(i)
		smallest = i
		if (l < self.size) and (degres[self.arr[l]] < degres[self.arr[i]]):
			smallest = l

		if (r < self.size) and (degres[self.arr[r]] < degres[self.arr[smallest]]):
			smallest = r

		if smallest != i:
			self.swap(i, smallest)
			self.minHeapify(smallest, degres)
