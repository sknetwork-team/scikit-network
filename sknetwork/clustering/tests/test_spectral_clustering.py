...
class TestSpectralClustering(unittest.TestCase):
    ...
    def test_default_options(self):
        ...
        spectral_clustering = SpectralClustering(n_components=3)
    ...
    def test_biclustering(self):
        ...
        bispectral_clustering = BiSpectralClustering(n_components=3, co_cluster=False)
        bispectral_clustering.fit(self.directed)
        self.assertEqual(bispectral_clustering.labels_.shape[0], self.directed.shape[0])
        self.assertTrue(bispectral_clustering.labels_col_ is None)

    def test_co_clustering(self):
        self.bipartite: sparse.csr_matrix = movie_actor()
        n1, n2 = self.bipartite.shape
        bispectral_clustering = BiSpectralClustering(n_components=3)
        bispectral_clustering.fit(self.bipartite)
        self.assertEqual(bispectral_clustering.labels_row_.shape[0], n1)
        self.assertEqual(bispectral_clustering.labels_col_.shape[0], n2)
