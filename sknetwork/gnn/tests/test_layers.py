...
class TestLayer(unittest.TestCase):
    ...
    def test_graph_conv_bias_use(self):
        conv1 = Convolution(4, use_bias=False)
        conv2 = Convolution(2, use_bias=False)
        h = conv1.forward(self.adjacency, self.features)
        emb = conv2.forward(self.adjacency, h)
        self.assertTrue(emb.shape == (self.adjacency.shape[0], 2))

    def test_graph_conv_self_loops(self):
        conv1 = Convolution(4, self_loops=False)
        conv2 = Convolution(2, self_loops=False)
        h = conv1.forward(self.adjacency, self.features)
        emb = conv2.forward(self.adjacency, h)
        self.assertTrue(emb.shape == (self.adjacency.shape[0], 2))

    def test_graph_conv_norm(self):
        conv1 = Convolution(4, normalization='left')
        conv2 = Convolution(2, normalization='right')
        h = conv1.forward(self.adjacency, self.features)
        emb = conv2.forward(self.adjacency, h)
        self.assertTrue(emb.shape == (self.adjacency.shape[0], 2))
    ...
    def test_graph_conv_activation(self):
        ...
        for a in activations:
            conv1 = Convolution(4, activation=a)
            conv2 = Convolution(2)
            h = conv1.forward(self.adjacency, self.features)
            emb = conv2.forward(self.adjacency, h)
            self.assertTrue(emb.shape == (self.adjacency.shape[0], 2))
